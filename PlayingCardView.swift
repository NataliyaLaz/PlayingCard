...
@IBDesignable//to be shown in main
class PlayingCardView: UIView {
    
    @IBInspectable//to be shown in Inspector
    var rank: Int = 12 {
        didSet {
    ...
    @objc func adjustFaceCardScale(byHandlingGestureRecognizedBy recognizer: UIPinchGestureRecognizer) {
        ...
        case .changed, .ended:
            ...
            recognizer.scale = 1.0 //not to resize exponentially
        default: break
        }
    }
    ...
    override func draw(_ rect: CGRect) {
        ...
        if isFaceUp {
            if let faceCardImage = UIImage(named: rankString + suit, in: Bundle(for: self.classForCoder), compatibleWith: traitCollection) {
                faceCardImage.draw(in: bounds.zoom(by: faceCardScale))//to work in FaceBuilder
            } else {
                drawPips()
            }
        } else {
